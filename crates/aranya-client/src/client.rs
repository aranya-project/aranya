...
use aranya_daemon_api::{CS, DaemonApiClient, DeviceId, KeyBundle, NetIdentifier, Role, TeamId};
...
use crate::{
    afc::{FastChannels, FastChannelsImpl, setup_afc_shm},
    error::{Error, Result},
};

/// A client for invoking actions on and processing effects from the Aranya graph.
///
/// `Client` interacts with the [Aranya daemon] via [`aranya-daemon-api`] ([`tarpc`] over Unix
/// domain sockets). The client provides AFC functionality by interfacing with AFC utilities in the
/// Aranya core crates.
///
...
impl Client {
    ...
    /// - `afc_shm_path`: AFC's shared memory path. The daemon must also use the same path.
    /// - `max_channels`: The maximum number of channels that AFC should support. The daemon must
    ///   also use the same number.
    /// - `afc_address`: The address that AFC listens for incoming connections on.
...
impl Team<'_> {
    ...
    ///
    /// This grants the device permission to send/receive AFC data using that label. A channel must
    /// be created with the label in order to send data using that label.
