...
impl Team<'_> {
    ...
    #[instrument(skip(self))]
    pub async fn assign_label_to_device(
        &self,
        device: DeviceId,
        label: LabelId,
        op: ChanOp,
    ...
    #[instrument(skip(self))]
    pub async fn revoke_label_from_device(&self, device: DeviceId, label: LabelId) -> Result<()> {
        self.client
            .daemon
            .revoke_label_from_device(
                context::current(),
                self.id,
                device.into_api(),
                label.into_api(),
...
impl Devices {
    /// Returns an iterator over the device IDs.
