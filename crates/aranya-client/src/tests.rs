...
use crate::{
    client::{ChanOp, Client, DeviceId, Permission, PublicKeyBundle, Role, TeamId},
    config::{CreateTeamConfig, SyncPeerConfig},
    text, AddTeamConfig, AddTeamQuicSyncConfig, Addr, CreateTeamQuicSyncConfig, Rank,
};
...
pub(crate) const SLEEP_INTERVAL: Duration = Duration::from_millis(250);
...
pub(crate) struct DevicesCtx {
    pub(crate) owner: DeviceCtx,
    pub(crate) admin: DeviceCtx,
    pub(crate) operator: DeviceCtx,
    pub(crate) membera: DeviceCtx,
    pub(crate) memberb: DeviceCtx,
    _work_dir: TempDir,
}
...
impl DevicesCtx {
    ...
    pub(crate) async fn add_all_device_roles(
        &mut self,
        team_id: TeamId,
        roles: &DefaultRoles,
    ) -> Result<()> {
    ...
    pub(crate) async fn create_and_add_team(&mut self) -> Result<TeamId> {
    ...
    async fn add_all_sync_peers(&self, team_id: TeamId) -> Result<()> {
...
impl DeviceCtx {
    ...
    pub(crate) async fn aranya_local_addr(&self) -> Result<Addr> {
...
async fn test_create_role() -> Result<()> {
    ...
    let rank = owner_team.query_rank(test_role.id).await?;
