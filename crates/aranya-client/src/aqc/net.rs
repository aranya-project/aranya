...
fn create_peer_channel(conn: Connection, channel_id: &AqcChannelId) -> AqcPeerChannel {
    match channel_id {
...
impl AqcClient {
    ...
    pub async fn receive_channel(&mut self) -> crate::Result<AqcPeerChannel> {
        loop {
            ...
            return Ok(create_peer_channel(conn, channel_info));
        }
    }
