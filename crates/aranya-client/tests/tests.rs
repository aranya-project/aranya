...
use aranya_client::{
    ...
    AddTeamConfig, AddTeamQuicSyncConfig, CreateTeamQuicSyncConfig, ObjectId, Rank,
};
...
async fn test_sync_now() -> Result<()> {
    ...
    owner
        .add_device_with_rank(devices.admin.pk.clone(), None, Rank::new(799))
        .await
        .context("owner unable to add admin to team")?;

    // Add the operator as a new device, but don't give it a role.
    owner
        .add_device_with_rank(devices.operator.pk.clone(), None, Rank::new(699))
        .await
...
async fn test_add_devices() -> Result<()> {
    ...
    admin
        .add_device_with_rank(team.operator.pk.clone(), None, Rank::new(699))
        .await
    ...
    for (name, kb, device_id) in [
        ...
        admin
            .add_device_with_rank(kb, None, Rank::new(500))
            .await
...
async fn test_remove_team() -> Result<()> {
...

    {
        ...
        match admin
            .create_label_with_rank(text!("test_label2"), Rank::new(500))
            .await
        {
            Ok(_) => bail!("Expected label creation to fail"),
            Err(aranya_client::Error::Aranya(_)) => {}
...
async fn test_setup_default_roles_single_use() -> Result<()> {
    ...
    let owner_team = devices.owner.client.team(team_id);
    match owner_team.setup_default_roles_no_owner().await {
...
async fn test_create_role() -> Result<()> {
    ...
    let test_role = owner_team
        .create_role(text!("test_role"), Rank::new(50))
        .await
...
async fn test_add_perm_to_created_role() -> Result<()> {
    ...
    let admin_role = owner_team
        .create_role(text!("admin"), Rank::new(500))
        .await?;
    owner_team
    ...
    owner_team
        .add_device_with_rank(devices.admin.pk, Some(admin_role.id), Rank::new(499))
        .await
    ...
    admin_team
        .add_device_with_rank(devices.operator.pk, None, Rank::new(400))
        .await
...
async fn test_privilege_escalation_rejected() -> Result<()> {
    ...
    owner_team
        .add_device_with_rank(device.pk.clone(), None, Rank::new(399))
        .await?;
    ...
    let role = owner_team
        .create_role(text!("malicious_role"), Rank::new(400))
        .await
    ...
    let target_role = device_team
        .create_role(text!("target_role"), Rank::new(300))
        .await
...
async fn test_remove_perm_from_default_role() -> Result<()> {
    ...
    owner_team
        .add_device_with_rank(devices.admin.pk, Some(roles.admin().id), Rank::new(799))
        .await
        .expect("expected to add admin with role");
    ...
    admin_team
        .add_device_with_rank(devices.operator.pk, None, Rank::new(500))
        .await
...
async fn test_delete_label_requires_permission() -> Result<()> {
    ...
    let label = owner_team
        .create_label_with_rank(text!("delete-label-guard"), Rank::new(500))
        .await?;
...
async fn test_assign_label_to_device_self_rejected() -> Result<()> {
    ...
    let label = owner_team
        .create_label_with_rank(text!("device-self-label"), Rank::new(500))
        .await?;
...
async fn test_change_rank_requires_sufficient_author_rank() -> Result<()> {
    ...
    let role_bytes: [u8; 32] = high_role.id.into();
    let role_obj: ObjectId = role_bytes.into();

    // Operator (rank 699) tries to change rank of object with rank 750 -- should fail
    match operator_team
        .change_rank(role_obj, Rank::new(750), Rank::new(600))
        .await
    {
...
async fn test_create_role_rank_too_high_rejected() -> Result<()> {
    ...
    match admin_team
        .create_role(text!("too_high"), Rank::new(900))
        .await
    {
...
async fn test_change_rank_above_role_rank_rejected() -> Result<()> {
    ...
    // Add admin with member role (rank 600) and device rank 599
    owner_team
        .add_device_with_rank(
            devices.admin.pk.clone(),
            Some(roles.member().id),
    ...
    let device_bytes: [u8; 32] = devices.admin.id.into();
    let device_obj: ObjectId = device_bytes.into();

    // Try to change device rank to 700 (above member role rank 600)
    // Should fail: role_rank (600) >= new_device_rank (700) is false
    match owner_team
        .change_rank(device_obj, Rank::new(599), Rank::new(700))
        .await
    {
...
async fn test_insufficient_rank_cannot_operate_on_objects() -> Result<()> {
    ...
    let high_label = owner_team
        .create_label_with_rank(text!("high_label"), Rank::new(800))
        .await?;
...
async fn test_change_rank_new_rank_above_author_rejected() -> Result<()> {
    ...
    // Owner creates a role at rank 50
    let role = owner_team
        .create_role(text!("low_role"), Rank::new(50))
        .await?;

    // Add operator (rank 699) with ChangeRank perm
    owner_team
        .add_device_with_rank(
            devices.operator.pk.clone(),
            Some(roles.operator().id),
    ...
    // Operator (rank 699) tries to change role rank from 50 to 800 (above operator's rank)
    match operator_team
        .change_rank(role_obj, Rank::new(50), Rank::new(800))
        .await
    {
...
async fn test_change_rank_stale_old_rank_rejected() -> Result<()> {
    ...
    let role = owner_team
        .create_role(text!("versioned_role"), Rank::new(50))
        .await?;
    let role_bytes: [u8; 32] = role.id.into();
    let role_obj: ObjectId = role_bytes.into();

    // Change rank from 50 to 75
    owner_team
        .change_rank(role_obj, Rank::new(50), Rank::new(75))
        .await?;

    // Try to change rank using stale old_rank (50 instead of current 75)
    match owner_team
        .change_rank(role_obj, Rank::new(50), Rank::new(100))
        .await
    {
...
async fn test_change_rank_self_demotion() -> Result<()> {
    ...
    // Add admin (rank 799) -- admin default role already has ChangeRank
    owner_team
        .add_device_with_rank(
            devices.admin.pk.clone(),
            Some(roles.admin().id),
    ...
    // Admin demotes itself from rank 799 to 500
    admin_team
        .change_rank(device_obj, Rank::new(799), Rank::new(500))
        .await
    ...
    assert_eq!(new_rank, Rank::new(500));
...
async fn test_assign_role_requires_outranking_both_role_and_device() -> Result<()> {
    ...
    owner_team
        .add_device_with_rank(devices.operator.pk.clone(), None, Rank::new(500))
        .await?;
...
async fn test_create_label_rank_too_high_rejected() -> Result<()> {
    ...
    match admin_team
        .create_label_with_rank(text!("too_high_label"), Rank::new(900))
        .await
    {
