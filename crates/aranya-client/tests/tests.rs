...
async fn test_role_create_assign_revoke() -> Result<()> {
    ...
    // Show that admin cannot create a label (no role assigned yet).
    admin_team
        .create_label(text!("label1"), roles.admin().id)
        .await
        .expect_err("expected label creation to fail");
...
/// With mTLS, devices can sync with any team they have the ID for - no add_team call needed.
#[test(tokio::test(flavor = "multi_thread"))]
async fn test_multi_team_sync() -> Result<()> {
    ...
    admin1
        ...
        .await
        .context("admin should be able to assign operator role on team1")?;

    // With mTLS, admin can also sync with team2 without calling add_team.
    ...
    admin2
        ...
        .await
        .context("admin should be able to assign operator role on team2")?;
