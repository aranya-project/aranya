...
impl DaemonApi for Api {
    ...
    async fn create_afc_uni_send_channel(
        ...
        let (channel_id, global_channel_id) = self.afc.uni_channel_created(e).await?;
        info!("afc uni channel created");

        let ctrl = get_afc_ctrl(ctrl)?;

        Ok((ctrl, channel_id, global_channel_id))
    }
    ...
    async fn receive_afc_ctrl(
        ...
        team: api::TeamId,
        ctrl: api::AfcCtrl,
    ) -> api::Result<(api::LabelId, api::AfcChannelId, api::AfcGlobalChannelId)> {
        self.check_team_valid(team).await?;
        ...
        let (channel_id, global_channel_id) = self.afc.uni_channel_received(e).await?;

        return Ok((e.label_id.into(), channel_id, global_channel_id));
    }
