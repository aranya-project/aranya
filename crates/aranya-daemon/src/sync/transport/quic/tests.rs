...
impl QuicTestSetup {
    ...
    fn load_device(paths: &CertPaths) -> (Vec<CertificateDer<'static>>, PrivateKeyDer<'static>) {
    ...
    fn build(
        ...
        let server_endpoint =
            quinn::Endpoint::server(server_config, server_addr).expect("failed to create server");
...
async fn test_serialized_message_exchange() {
    ...
    let server_handle = tokio::spawn(async move {
        ...
        let response_data = postcard::to_allocvec(&response).expect("failed to serialize response");
