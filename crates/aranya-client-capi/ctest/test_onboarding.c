...
static int test_create_team_and_onboard_member(void) {
    ...
    err = aranya_client_config_builder_set_daemon_uds_path(&owner_builder,
                                                           g_owner_uds);
    if (err != ARANYA_ERROR_SUCCESS) {
    ...
    err = aranya_client_config_builder_set_daemon_uds_path(&member_builder,
                                                           g_member_uds);
    if (err != ARANYA_ERROR_SUCCESS) {
    ...
    err = aranya_add_device_to_team(&owner_client, &team_id, member_pk,
                                    member_pk_len, NULL);
    if (err != ARANYA_ERROR_SUCCESS) {
