...
use aranya_client::{
    AddTeamConfig, Addr, ChanOp, Client, CreateTeamConfig, DeviceId, PublicKeyBundle, Rank, afc,
    text,
};
...
async fn run_demo_body(ctx: DemoContext) -> Result<()> {
    ...
    let admin_role_rank = owner.rank(admin_role.id).await?;
    owner
        .add_device(
            ctx.admin.pk,
            Some(admin_role.id),
    ...
    let operator_role_rank = owner.rank(operator_role.id).await?;
    owner
        .add_device(
            ctx.operator.pk,
    ...
    let member_role_rank = admin.rank(member_role.id).await?;
    admin
        .add_device(
    ...
    let admin_device_rank = admin.rank(ctx.admin.id).await?;
