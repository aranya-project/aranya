...
async fn main() -> Result<()> {
    ...
    // Add team to the other devices' storage using the init command
    // generated by the owner
    let cfg = TeamConfig::builder().init_command(&init_cmd).build()?;

    for team in [&admin_team, &operator_team, &membera_team, &memberb_team] {
        team.add_team_to_device(&cfg).await?;
    }
